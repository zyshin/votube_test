...
if (['4', '10'].indexOf(params.pid) >= 0) {
    ...
    // TASKS[17] = '';  // mcc
    if (TASKS.length == 20) {
        TASKS.push('3020003000100100013020100003013001300002230100003000000220031122030020100001212030000203'); // dnx
    }
    if (TASKS.length == 21) {
        TASKS.push('0002000030320300030110000000320101000200100203000000132020002123100013200201132001300300'); // hpk
    }
    if (TASKS.length == 22) {
        TASKS.push('0301200221010200310320000003120002000302100101002001130210002300300000300000303000300201'); // lss
    }
    if (TASKS.length == 23) {
        TASKS.push('0311000210100030000221000003213131000103002000003020010220031020030020000000100332203000'); // zpj
    }
    TASKS[7] = '0010300321230200310110000000300201000203300102030002112330002230000010200002101000000300';  // pxy
    TASKS[3] = '0022000002310300120113000000120302000000100203000013022010003310300020300003232001100100';  // hy
    TASKS[17] = '0020003002120103200020000003020102000130203101030001310000031020100000100001203033200230'; // lxh
}
